...
int main(int argc, char **argv)
{
	...
	ppg->arch_data = calloc(1, sizeof(struct cygwin_arch_data));
	...
	/* Set offset here, so config parsing can override it. Since adjtimex not available in cygwin 
	and we don't know the offset we set the current offset to zero*/
	timePropertiesDS.currentUtcOffset = 0;
